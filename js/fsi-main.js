...
const CDN_CONFIG = {
   ...
   baseUrl: 'https://rhodesagi.github.io/rhodes-french-gsi-enhanced/',  // GitHub Pages playground mode
   ...
   data: {
     ...
     course: 'data/course.json',           // Linear mode course structure
     exerciceStructure: 'data/exercice_structure.json' // FSI Volume 2 exercise grouping (Units 13-24)
   },

  // Audio files
  // For the playground site, keep audio served from the stable fsi-enhanced Pages path
  // so we can deploy without pushing ~200MB of MP3s every iteration.
  audio: {
    dialogues: 'https://rhodesagi.github.io/rhodes-french-fsi-enhanced/audio/',          // unit01_dialogue.mp3, etc.
    drills: 'https://rhodesagi.github.io/rhodes-french-fsi-enhanced/audio/drills/'       // unit01_0001_fr.mp3, unit01_0001_en.mp3, etc.
  },
...
function cdnUrl(path) {
  ...
  return `${CDN_CONFIG.baseUrl}${path}?v=${version}`;
}
...
let fsiExerciseStructure = null;
let fsiDrillFormatPatch = null;
...
let linearDrillSession = false;  // True when drilling in linear mode (not SRS)
let autoDirectionGuard = false;
let sessionSequencingMode = null; // resolved per session

const SEQUENCING = {
  STORAGE_KEY: 'rhodes_sequence_mode',
  Modes: {
...
function getSequenceMode() {
  ...
  const override = params.get('seq');
  if (override === SEQUENCING.Modes.Mixed || override === SEQUENCING.Modes.Audiolingual) return override;
  try {
    const saved = localStorage.getItem(SEQUENCING.STORAGE_KEY);
    if (saved === SEQUENCING.Modes.Mixed || saved === SEQUENCING.Modes.Audiolingual) return saved;
  } catch {}
  return SEQUENCING.Modes.Audiolingual;
}

function setSequenceMode(mode) {
  if (mode !== SEQUENCING.Modes.Mixed && mode !== SEQUENCING.Modes.Audiolingual) return;
  try {
    localStorage.setItem(SEQUENCING.STORAGE_KEY, mode);
  } catch {}
}

function getSequenceParamsForUnit(unitId) {
  const u = Number(unitId) || 1;
  // Strong grouping early; gradually reduce as units progress.
  const start = Math.max(2, Math.round(8 - u * 0.35));
  const end = 1;
  const params = new URLSearchParams(window.location.search);
  const startOverride = parseInt(params.get('groupStart') || '', 10);
  const endOverride = parseInt(params.get('groupEnd') || '', 10);
  return {
    blockStart: Number.isFinite(startOverride) ? Math.max(1, startOverride) : start,
    blockEnd: Number.isFinite(endOverride) ? Math.max(1, endOverride) : end,
    spacedRepeat: params.get('spaced') === '0' ? false : true
  };
}

function drillFormat(drill) {
  if (!drill) return 'unknown';
  if (typeof drill.drill_format === 'string' && drill.drill_format.trim()) return drill.drill_format.trim();
  if (typeof drill.type === 'string' && drill.type.startsWith('fsi_')) return drill.type.replace('fsi_', '');
  return 'sentence';
}
...
function stableOrderIndex(drill) {
  const id = drill?.id;
...
function groupKeyForDrill(drill) {
  const fmt = drillFormat(drill);
  const isFsi = typeof drill?.type === 'string' && drill.type.startsWith('fsi_');
  if (isFsi) {
    const model = drill.model_sentence || (Array.isArray(drill.expected_responses) ? drill.expected_responses[0] : '') || drill.french_formal || '';
    return `${fmt}|${normalizeKey(model)}`;
  }
  const pg = drill.pattern_group || drill.pos_pattern || '';
  return `${fmt}|${normalizeKey(pg || drill.french_formal || drill.french || drill.id || '')}`;
}

function buildAudiolingualOrder(drills, unitId, opts) {
  const groups = new Map(); // key -> { key, items: [] }
  for (const d of drills) {
    const key = groupKeyForDrill(d);
  ...
  const groupList = Array.from(groups.values()).map(g => {
    g.items.sort((a, b) => stableOrderIndex(a) - stableOrderIndex(b));
    return g;
  });

  // Sort groups by earliest member index for stable progression.
  groupList.sort((a, b) => stableOrderIndex(a.items[0]) - stableOrderIndex(b.items[0]));
  ...
  const out = [];
  const firstSeenPos = new Map(); // id -> outIndex
  const spacedQueue = []; // drills in first-seen order (unique ids)
  const repeated = new Set(); // ids already repeated
  let spacedCursor = 0;
  let blocks = 0;
  ...
  while (consumed < total) {
    ...
    let g = null;
    for (let tries = 0; tries < groupList.length; tries++) {
      const idx = (groupIdx + tries) % groupList.length;
      if (groupList[idx].items.length > 0) {
        g = groupList[idx];
        groupIdx = (idx + 1) % groupList.length;
        break;
      }
    }
    if (!g) break;
    const currentFormat = typeof g.key === 'string' ? g.key.split('|')[0] : null;

    const progress = total > 0 ? consumed / total : 1;
    const blockSize = Math.max(1, Math.round(opts.blockStart * (1 - progress) + opts.blockEnd * progress));
    const take = g.items.splice(0, blockSize);
    for (const d of take) {
      out.push(d);
      consumed += 1;
      if (!firstSeenPos.has(d.id)) {
        firstSeenPos.set(d.id, out.length - 1);
        spacedQueue.push(d);
      }
    }

    // Insert a single spaced repeat from earlier in the same format (without touching SRS scheduling).
    if (opts.spacedRepeat && spacedQueue.length > 0) {
      const spacingMin = Math.min(14, 3 + Math.floor(out.length / 12));
      // Only start repeating after the session has some runway.
      const shouldInsert = out.length >= spacingMin * 2 && (blocks % 2 === 1 || progress > 0.35);
      if (shouldInsert) {
        for (let tries = 0; tries < spacedQueue.length; tries++) {
          const candidate = spacedQueue[spacedCursor % spacedQueue.length];
          spacedCursor += 1;
          if (!candidate?.id || repeated.has(candidate.id)) continue;
          if (currentFormat && drillFormat(candidate) !== currentFormat) continue;
          const pos = firstSeenPos.get(candidate.id);
          if (typeof pos !== 'number') continue;
          if ((out.length - 1) - pos < spacingMin) continue;
          out.push({ ...candidate, is_repeat: true });
...
function hasUsableEnglish(drill) {
  const english = drill?.english;
  if (typeof english !== 'string') return false;
  const text = english.trim();
  if (!text) return false;
  if (text.startsWith('[')) return false; // metadata like [Response drill: ...]
  if (/translation needed/i.test(text)) return false;
  return true;
}

function ensureDirectionIsValidForDrill(drill) {
  if (autoDirectionGuard) return;
  const isFsi = typeof drill?.type === 'string' && drill.type.startsWith('fsi_');
  if (drillDirection === 'fr-en' && (isFsi || !hasUsableEnglish(drill))) {
    autoDirectionGuard = true;
    setDirection('en-fr');
    autoDirectionGuard = false;
  }
}

function guessFsiPersonPlaceholder(sentence) {
  const s = (sentence || '').toLowerCase();
  if (/\belle\b/.test(s) || /s'elle\b/.test(s) || /qu'elle\b/.test(s)) return 'cette dame';
  return 'ce monsieur';
}

function fillFsiBlankSlots(sentence) {
  if (typeof sentence !== 'string') return sentence;
  const t = sentence;
  const hasDots = t.includes('...') || /[.]\s*[•·]{2,}/.test(t);
  const hasGap = /\b(Demandez|Dites)\s+à\s{2,}/.test(t);
  if (!hasDots && !hasGap) return sentence;

  const who = guessFsiPersonPlaceholder(t);
  return t
    .replace(/à\s*\.\.\.\s*/g, `à ${who} `)
    .replace(/à\s*\.\s*[•·]{2,}\s*/g, `à ${who} `)
    .replace(/\b(Demandez|Dites)\s+à\s{2,}/g, `$1 à ${who} `)
    .replace(/\s+/g, ' ')
    .trim();
}
...
function EnglishCompare(userText, expectedText) {
  const user = normalizeEnglish(userText);
  const expected = normalizeEnglish(expectedText);
  return {
    correct: user.length > 0 && user === expected,
    accentWarning: false,
    errors: user === expected ? [] : ['mismatch'],
    normalized: { user, expected }
  };
}

function tryIncrementFsiTrailingIndex(id) {
  // e.g. fsi_response_unit3_sec26_021_006 -> ..._007
...
function mergeSplitFsiVolume1Lines(drillsDataObj, courseDataObj) {
  if (!drillsDataObj?.drills || !Array.isArray(drillsDataObj.drills)) return;
  const drillsById = new Map(drillsDataObj.drills.map(d => [d.id, d]));

  const toSkip = new Set();
  for (const d of drillsDataObj.drills) {
    if (!d || typeof d !== 'object') continue;
    if (!(typeof d.type === 'string' && d.type.startsWith('fsi_'))) continue;
    if (d.unit >= 13) continue; // Volume 1 only
    if (d.drill_format !== 'response') continue;

    const model = (d.model_sentence || '').trim();
    const expected0 = Array.isArray(d.expected_responses) && typeof d.expected_responses[0] === 'string' ? d.expected_responses[0].trim() : '';
    if (!model || model !== expected0) continue;
    if (!/^(Demandez|Dites)\s+à$/i.test(model)) continue;

    const nextId = tryIncrementFsiTrailingIndex(d.id);
    if (!nextId) continue;
    const next = drillsById.get(nextId);
    if (!next) continue;

    const nextModel = (next.model_sentence || '').trim();
    const nextExpected0 = Array.isArray(next.expected_responses) && typeof next.expected_responses[0] === 'string' ? next.expected_responses[0].trim() : '';
    if (nextModel !== '' || !nextExpected0) continue;

    // Join with an explicit blank slot marker; later we fill it with a safe placeholder.
    const joined = `${model} ... ${nextExpected0}`.replace(/\s+/g, ' ').trim();
    d.model_sentence = joined;
    d.french_formal = joined;
    d.french_informal = joined;
    d.expected_responses = [joined];

    toSkip.add(nextId);
  }

  if (toSkip.size > 0) {
    drillsDataObj.drills = drillsDataObj.drills.filter(d => !toSkip.has(d.id));
    drillsDataObj.total_drills = drillsDataObj.drills.length;
  }

  // Keep course.json drill copies consistent with the merged drills.json objects.
  if (courseDataObj?.units && Array.isArray(courseDataObj.units)) {
    const freshById = new Map(drillsDataObj.drills.map(d => [d.id, d]));
    for (const unit of courseDataObj.units) {
      const drills = unit?.drills;
      if (!drills || typeof drills !== 'object') continue;
      for (const list of Object.values(drills)) {
        if (!Array.isArray(list)) continue;
        // Remove drills that were split-continuations we merged away.
        if (toSkip.size > 0) {
          for (let i = list.length - 1; i >= 0; i--) {
            const id = list[i]?.id;
            if (typeof id === 'string' && toSkip.has(id)) list.splice(i, 1);
          }
        }
        for (const d of list) {
          const src = d?.id ? freshById.get(d.id) : null;
          if (!src) continue;
          if (typeof src.model_sentence === 'string') d.model_sentence = src.model_sentence;
          if (Array.isArray(src.cues)) d.cues = src.cues;
          if (Array.isArray(src.expected_responses)) d.expected_responses = src.expected_responses;
          if (typeof src.french_formal === 'string') d.french_formal = src.french_formal;
          if (typeof src.french_informal === 'string') d.french_informal = src.french_informal;
        }
      }
    }
  }
...
function setCardLinkForCurrentDrill(drill) {
  const cardLink = document.getElementById('cardLink');
  if (!cardLink || !drill?.id) return;
  cardLink.href = buildCardUrl(drill.id);
  cardLink.textContent = `Link to this drill`;
  cardLink.style.display = 'inline-block';
}

function maybeTrackCardInUrl(drill) {
  if (!drill?.id) return;
  const params = new URLSearchParams(window.location.search);
  if (params.get('trackCard') !== '1') return;
  const url = new URL(window.location.href);
  url.searchParams.set('card', drill.id);
  history.replaceState(null, '', url.toString());
}

function openCardFromUrl() {
  const cardId = new URLSearchParams(window.location.search).get('card');
  if (!cardId || !drillsData?.drills) return false;
  const drill = drillsData.drills.find(d => d?.id === cardId);
  if (!drill) {
    const grid = document.getElementById('unitsGrid');
    if (grid) {
      grid.insertAdjacentHTML('afterbegin', `
        <div style="grid-column: 1/-1; padding: 12px; background: #f8d7da; border: 2px solid #dc3545; margin-bottom: 10px;">
          <strong>Card not found</strong><br>
          Unknown card id: <code>${cardId}</code>
        </div>
      `);
    }
    return false;
  ...
  // Start a single-card debug session
  currentMode = 'debug';
  currentUnit = null;
  currentDrills = [drill];
  currentDrillIndex = 0;
  sessionCorrect = 0;
  sessionTotal = 0;
  linearDrillSession = false;
  showDrill();
  return true;
}

// ===========================================
// FSI DRILL FORMAT REPAIR (Volume 2)
// ===========================================
// The raw OCR for Volume 2 contains noise (e.g. stray "h.") that can corrupt
// `model_sentence` / `cues`. We repair these in-memory using
// `exercice_structure.json` + each drill's `legacy_id` mapping.

const FSI_DrillFormats = {
  debug: new URLSearchParams(window.location.search).get('debugFsi') === '1',

  parseLegacyId(legacyId) {
    // Example: fsi_substitution_A-2_001  => { exerciceId: 'A2', drillNumber: 1 }
    //          fsi_transformation_B-10_004 => { exerciceId: 'B10', drillNumber: 4 }
    if (!legacyId || typeof legacyId !== 'string') return null;
    const m = legacyId.match(/^fsi_[^_]+_([A-Z])-(\d+)_0*(\d+)$/);
    if (!m) return null;
    const exerciceId = `${m[1]}${parseInt(m[2], 10)}`;
    const drillNumber = parseInt(m[3], 10);
    if (!Number.isFinite(drillNumber)) return null;
    return { exerciceId, drillNumber };
  },
  ...
  isBadModelSentence(text) {
    const t = (text || '').trim();
    if (!t) return true;
    if (t.length < 5) return true;
    if (!this.hasRealLetters(t)) return true;
    // Common OCR garbage tokens seen in Volume 2
    if (/^[a-z]\.$/i.test(t)) return true; // e.g. "h."
  ...
  tokenize(text) {
    if (!text || typeof text !== 'string') return [];
    // Words (with apostrophes) + punctuation tokens
  ...
  extractCue(modelSentence, responseSentence) {
    const modelNorm = this.normalizeText(modelSentence);
    const responseNorm = this.normalizeText(responseSentence);
    const modelTokensNorm = this.tokenize(modelNorm);
    const responseTokensNorm = this.tokenize(responseNorm);
    const responseTokensOrig = this.tokenize(responseSentence);
    if (modelTokensNorm.length === 0 || responseTokensNorm.length === 0 || responseTokensOrig.length === 0) return '';
    // Safety: ensure indices line up between normalized and original token lists.
    if (responseTokensNorm.length !== responseTokensOrig.length) return '';

    let start = 0;
    while (start < modelTokensNorm.length && start < responseTokensNorm.length && modelTokensNorm[start] === responseTokensNorm[start]) {
      start += 1;
    }

    let end = 0;
    while (
      end < (modelTokensNorm.length - start) &&
      end < (responseTokensNorm.length - start) &&
      modelTokensNorm[modelTokensNorm.length - 1 - end] === responseTokensNorm[responseTokensNorm.length - 1 - end]
    ) {
      end += 1;
    }

    const cueTokens = responseTokensOrig.slice(start, responseTokensOrig.length - end);
    const cue = this.joinTokens(cueTokens);
    if (!cue) return '';
    const cueNorm = this.normalizeText(cue);
    if (!cueNorm) return '';
    if (cueNorm === this.normalizeText(modelSentence) || cueNorm === this.normalizeText(responseSentence)) return '';
    // If the "cue" is basically the whole sentence, it isn't useful.
    if (cue.length > 120) return '';
    return cue;
  },

  buildExerciseIndex(structure) {
    const byKey = new Map();      // `${unit}|${exerciceId}|${number}` -> candidates[]
    const byExercise = new Map(); // `${unit}|${exerciceId}` -> candidates[]
    if (!structure || typeof structure !== 'object' || !Array.isArray(structure.exercices)) {
      return { byKey, byExercise };
    }

    for (const ex of structure.exercices) {
      const unit = ex?.unit;
      const exerciceId = ex?.exercice_id;
      const drills = ex?.drills;
      if (!Number.isFinite(unit) || !exerciceId || !Array.isArray(drills) || drills.length === 0) continue;

      const canonical = drills.find(d => d?.is_canonical && typeof d?.french === 'string' && d.french.trim()) ||
                        drills.find(d => typeof d?.french === 'string' && d.french.trim());
      const modelSentence = canonical?.french?.trim() || '';
      if (!this.hasRealLetters(modelSentence)) continue;

      for (const d of drills) {
        const number = d?.number;
        const response = d?.french?.trim();
        if (!Number.isFinite(number) || !response) continue;
        const candidate = {
          modelSentence,
          response,
          responseNorm: this.normalizeText(response)
        };

        const key = `${unit}|${exerciceId}|${number}`;
        const candidatesByKey = byKey.get(key) || [];
        candidatesByKey.push(candidate);
        byKey.set(key, candidatesByKey);

        const exKey = `${unit}|${exerciceId}`;
        const candidatesByExercise = byExercise.get(exKey) || [];
        candidatesByExercise.push(candidate);
        byExercise.set(exKey, candidatesByExercise);
      }
    }
    return { byKey, byExercise };
  },

  pickBestCandidate(candidates, responseSentence) {
    if (!Array.isArray(candidates) || candidates.length === 0) return null;
    const responseNorm = this.normalizeText(responseSentence);
    if (!responseNorm) return candidates[0] || null;

    // Prefer exact match by normalized response
    const exact = candidates.find(c => c.responseNorm === responseNorm);
    if (exact) return { candidate: exact, match: 'exact' };

    // Otherwise choose candidate with highest token overlap
    const responseTokens = new Set(this.tokenize(responseNorm));
    let best = candidates[0];
    let bestScore = -1;
    for (const c of candidates) {
      const candTokens = new Set(this.tokenize(c.responseNorm));
      if (candTokens.size === 0 || responseTokens.size === 0) continue;
      let intersection = 0;
      for (const t of candTokens) if (responseTokens.has(t)) intersection += 1;
      const score = intersection / Math.max(1, Math.max(candTokens.size, responseTokens.size));
      if (score > bestScore) {
        bestScore = score;
        best = c;
      }
    }
    return { candidate: best, match: 'overlap' };
  },

  applyToDrill(drill, exerciseIndex) {
    if (!drill || typeof drill !== 'object') return;
    if (!(typeof drill.type === 'string' && drill.type.startsWith('fsi_'))) return;
    if (!(Number.isFinite(drill.unit) && drill.unit >= 13)) return;

    const legacy = this.parseLegacyId(drill.legacy_id);
    if (!legacy) return;

    const responseSentence =
      (Array.isArray(drill.expected_responses) && drill.expected_responses[0]) ||
      drill.french_formal ||
      '';
    if (!this.hasRealLetters(responseSentence)) return;

    const key = `${drill.unit}|${legacy.exerciceId}|${legacy.drillNumber}`;
    let candidates = exerciseIndex?.byKey?.get(key);
    let source = 'byKey';
    if (!candidates || candidates.length === 0) {
      const exKey = `${drill.unit}|${legacy.exerciceId}`;
      candidates = exerciseIndex?.byExercise?.get(exKey);
      source = 'byExercise';
    }
    if (!candidates || candidates.length === 0) return;

    const selection = this.pickBestCandidate(candidates, responseSentence);
    const candidate = selection?.candidate;
    const modelSentence = candidate?.modelSentence || '';
    if (!this.hasRealLetters(modelSentence)) return;

    drill.model_sentence = modelSentence;

    const cue = this.extractCue(modelSentence, responseSentence);
    drill.cues = cue ? [cue] : [];

    if (this.debug) {
      drill.fsi_repair = {
        source,
        match: selection?.match || 'unknown',
        unit: drill.unit,
        exercice_id: legacy.exerciceId,
  ...
  apply(drillsDataObj, courseDataObj, exerciseStructureObj) {
    if (!drillsDataObj?.drills || !Array.isArray(drillsDataObj.drills)) return;
    const exerciseIndex = this.buildExerciseIndex(exerciseStructureObj);
    if (exerciseIndex.byKey.size === 0 && exerciseIndex.byExercise.size === 0) return;

    // Patch drills.json objects in-place
    for (const drill of drillsDataObj.drills) {
      this.applyToDrill(drill, exerciseIndex);
    }

    // Patch course.json objects (separate copies) to keep linear mode consistent
    const drillsById = new Map(drillsDataObj.drills.map(d => [d.id, d]));
    if (courseDataObj?.units && Array.isArray(courseDataObj.units)) {
      for (const unit of courseDataObj.units) {
        const drills = unit?.drills;
        ...
        for (const list of Object.values(drills)) {
          if (!Array.isArray(list)) continue;
          for (const d of list) {
            const src = d?.id ? drillsById.get(d.id) : null;
            if (!src) continue;
            if (typeof src.model_sentence === 'string') d.model_sentence = src.model_sentence;
            if (Array.isArray(src.cues)) d.cues = src.cues;
          }
...
async function loadCourse() {
  try {
    ...
    console.log('Storage initialized');
     ...
     // FSI_Linear will be initialized after course data is loaded
  } catch (initErr) {
  ...
  try {
     ...
     // Load course data for linear mode
     console.log('Loading course data...');
     const courseRes = await fetchWithFallback(CDN_CONFIG.data.course);
     courseData = await courseRes.json();
     console.log('Loaded course data:', courseData.units?.length || 0, 'units');

     // Load FSI Volume 2 exercise structure (for repairing drill formats)
     try {
       const exRes = await fetchWithFallback(CDN_CONFIG.data.exerciceStructure);
       fsiExerciseStructure = await exRes.json();
       console.log('Loaded exercice structure:', fsiExerciseStructure.exercices?.length || 0, 'exercices');
       FSI_DrillFormats.apply(drillsData, courseData, fsiExerciseStructure);
       console.log('Applied FSI drill format repairs');
     } catch (exErr) {
       console.warn('Exercice structure unavailable; FSI drill prompts may be degraded:', exErr.message);
     }

     // Optional: apply an override patch file (for experimentation)
     // Format: { patch: { [drillId]: { model_sentence, cues } } }
     // Enable via URL param: ?fsiPatch=1
     if (new URLSearchParams(window.location.search).get('fsiPatch') === '1') {
       try {
         const patchRes = await fetchWithFallback('data/fsi_drill_format_patch.json');
         fsiDrillFormatPatch = await patchRes.json();
         const patch = fsiDrillFormatPatch?.patch;
         if (patch && typeof patch === 'object') {
           // Apply to drillsData
           const drillsById = new Map(drillsData.drills.map(d => [d.id, d]));
           for (const [id, v] of Object.entries(patch)) {
             const d = drillsById.get(id);
             if (!d) continue;
             if (typeof v?.model_sentence === 'string') d.model_sentence = v.model_sentence;
             if (Array.isArray(v?.cues)) d.cues = v.cues;
           }
           // Apply to courseData drills as well
           ...
           console.log('Applied FSI drill format patch file:', Object.keys(patch).length, 'entries');
         }
       } catch (patchErr) {
         console.warn('FSI patch file not found or failed to load:', patchErr.message);
       }
     }

     // Fix known Volume 1 PDF extraction artifacts (multi-line Response Drill entries split into separate drills)
     mergeSplitFsiVolume1Lines(drillsData, courseData);

     // If URL includes a card id, jump directly to that drill for debugging.
     openCardFromUrl();
    ...
    try {
      ...
      console.log('Loaded main audio mapping:', Object.keys(audioMapping).length, 'mappings');
      
      // Load FSI audio mapping and merge
      try {
        const fsiMappingRes = await fetchWithFallback('data/fsi_audio_mapping.json');
        const fsiAudioMapping = await fsiMappingRes.json();
        console.log('Loaded FSI audio mapping:', Object.keys(fsiAudioMapping).length, 'mappings');
        
        // Merge FSI mapping into main mapping (FSI IDs start with 'fsi_', so no conflicts)
        Object.assign(audioMapping, fsiAudioMapping);
        console.log('Combined audio mapping:', Object.keys(audioMapping).length, 'total mappings');
      } catch (fsiError) {
        console.warn('FSI audio mapping not found, continuing with main mapping only:', fsiError.message);
      }
    } catch (e) {
     ...
     // Initialize FSI_Linear with course data
     if (typeof FSI_Linear !== 'undefined' && courseData) {
       await FSI_Linear.init(courseData);
       console.log('FSI_Linear initialized with course data');
     }
...
function openUnit(unitId) {
  ...
  const drillsSection = document.getElementById('drillsSection');
  if (unit.noDrills || unitDrills.length === 0) {
    ...
    // Get unit data from courseData for drill type breakdown
    const courseUnit = courseData?.units?.find(u => u.id === unitId);
    const drillTypes = [
      { key: 'lexical', name: 'Lexical Drills', cssClass: 'fsi-lexical', section: 'lexical_drills' },
      { key: 'substitution', name: 'Substitution Drills', cssClass: 'fsi-substitution', section: 'substitution_drills' },
      { key: 'transformation', name: 'Transformation Drills', cssClass: 'fsi-transformation', section: 'transformation_drills' },
      { key: 'response', name: 'Response Drills', cssClass: 'fsi-response', section: 'response_drills' },
      { key: 'expansion', name: 'Expansion Drills', cssClass: 'fsi-review', section: 'expansion_drills' },
      { key: 'variation', name: 'Variation Drills', cssClass: 'fsi-variation', section: 'variation_drills' },
      { key: 'translation', name: 'Translation Drills', cssClass: 'fsi-question', section: 'translation_drills' },
      { key: 'review', name: 'Review Drills', cssClass: 'fsi-review', section: 'review' }
    ];
    
    let gridHtml = '<div class="drill-type-grid">';
    drillTypes.forEach(type => {
      const drills = courseUnit?.drills?.[type.key] || [];
      const count = drills.length;
      const disabled = count === 0 ? 'empty' : '';
      gridHtml += `
        <button class="drill-type-btn ${type.cssClass} ${disabled}" 
                data-unit="${unitId}" 
                data-type="${type.key}"
                data-section="${type.section}"
                ${count === 0 ? 'disabled' : ''}>
          ${type.name} <span class="count">${count}</span>
        </button>
      `;
    });
    gridHtml += '</div>';
    
    // Add full unit button
    gridHtml += `
      <button class="btn btn-primary" style="width: 100%; font-size: 20px; padding: 15px; margin-top: 20px;" id="startDrillsBtn">
        FULL UNIT: ALL DRILLS (${unitDrills.length}) &#10140;
      </button>
    `;
    
  drillsSection.innerHTML = `
      <h3 style="margin-bottom: 10px; color: var(--accent-blue);">&#9998; DRILLS</h3>
      <div style="display:flex; gap:12px; align-items:center; flex-wrap:wrap; margin: 0 0 12px 0;">
        <div style="color:#666;">Sequence:</div>
        <select id="sequenceMode" style="padding: 8px; font-family: inherit; font-size: 14px;">
          <option value="audiolingual">Audiolingual (grouped → interleaved)</option>
          <option value="mixed">Mixed (unseen by commonality)</option>
        </select>
        <div style="color:#666; font-size: 12px;">(Override via URL: <code>?seq=audiolingual</code> or <code>?seq=mixed</code>)</div>
      </div>
      <p style="color: #666; margin-bottom: 15px;">Select drill type to practice:</p>
      ${gridHtml}
    `;

    const sel = document.getElementById('sequenceMode');
    if (sel) {
      sel.value = getSequenceMode();
      sel.addEventListener('change', () => setSequenceMode(sel.value));
    }
    
    // Add click handlers for drill type buttons
    document.querySelectorAll('.drill-type-btn:not(.empty)').forEach(btn => {
      btn.addEventListener('click', () => {
        const unitId = parseInt(btn.dataset.unit);
        const section = btn.dataset.section;
        startDrillTypePractice(unitId, section);
      });
    });
    
    // Keep existing startDrillsBtn handler
    document.getElementById('startDrillsBtn')?.addEventListener('click', () => {
      startUnitDrills();
    });
  }
...
function startUnitDrills() {
  linearDrillSession = false;  // SRS mode, not linear
  loadUnitDrills(currentUnit);
}

// Start Linear mode with specific drill type
function startDrillTypePractice(unitId, section) {
  if (typeof FSI_Linear === 'undefined' || !FSI_Linear) {
    console.error('FSI_Linear not available');
    alert('Linear mode not available. Please refresh the page.');
    return;
  }
  
  // Set linear state
  FSI_Linear.goTo(unitId, section, 0);
  
  // Get drills for this section
  const sectionData = FSI_Linear.getCurrentSectionData();
  if (!sectionData || sectionData.length === 0) {
    alert(`No drills available for ${section} in Unit ${unitId}.`);
    return;
  }
  
  // Convert to drill format (add french field)
  const drillsForDisplay = sectionData.map(d => ({
    ...d,
    french: d.french_formal || d.french || '',
    seen: false  // Not used for linear mode but keep compatibility
  }));
  
  const mode = getSequenceMode();
  sessionSequencingMode = mode;
  if (mode === SEQUENCING.Modes.Audiolingual) {
    const opts = getSequenceParamsForUnit(unitId);
    currentDrills = buildAudiolingualOrder(drillsForDisplay, unitId, opts);
  } else {
    currentDrills = drillsForDisplay;
  }
  
  // Set linear drill session flag
  linearDrillSession = true;
  currentDrillIndex = 0;
  sessionCorrect = 0;
  sessionTotal = 0;
...
function startDialogueDrills() {
  linearDrillSession = false;  // Dialogue drills are not linear mode
  const unit = UNIT_DATA[currentUnit];
...
function loadUnitDrills(unitId) {
  ...
  // Convert to display format - preserve all fields, add french and seen
  const drillsForDisplay = unitDrills.map(d => ({
    ...d,
  ...
  if (mode === SEQUENCING.Modes.Audiolingual) {
    const opts = getSequenceParamsForUnit(unitId);
    const unseen = drillsForDisplay.filter(d => !d.seen);
    const seen = drillsForDisplay.filter(d => d.seen);

    // Group unseen drills in an audiolingual-first progression, then append seen drills.
    currentDrills = [
      ...buildAudiolingualOrder(unseen, unitId, opts),
      ...seen
    ];
  } else {
    currentDrills = drillsForDisplay;
...
function startSRSSession() {
  ...
  currentDrills = drillsData.drills.map(d => {
  ...
  currentDrills.sort((a, b) => {
    ...
    // Within new: sort by commonality (most common first) with FSI priority
    if (a.srsState === 'new' && b.srsState === 'new') {
      const aFSIBonus = a.type && a.type.startsWith('fsi_') ? 10 : 0;
      const bFSIBonus = b.type && b.type.startsWith('fsi_') ? 10 : 0;
      const aScore = a.commonality + (a.isReviewUnit ? -0.1 : 0) + aFSIBonus;
      const bScore = b.commonality + (b.isReviewUnit ? -0.1 : 0) + bFSIBonus;
...
function updateDrillDisplay() {
  ...
  // Some drills (notably FSI format drills) do not support FR→EN mode reliably.
  ensureDirectionIsValidForDrill(drill);
  setCardLinkForCurrentDrill(drill);
  maybeTrackCardInUrl(drill);
  ...
  if (typeof FSI_SRS !== 'undefined') {
   ...
   document.getElementById('drillProgress').textContent = `${currentDrillIndex + 1} / ${currentDrills.length}${drill.is_repeat ? ' (repeat)' : ''}`;

    // FSI drill detection and styling
    const drillTypeElement = document.getElementById('drillType');
    if (drill.type && drill.type.startsWith('fsi_')) {
      drillTypeElement.classList.add('fsi-drill');
      drillTypeElement.classList.add('fsi-' + drill.type.replace('fsi_', ''));
      // Show FSI-specific hints if available
      const grammarHints = document.getElementById('grammarHints');
      const hintsContent = document.getElementById('hintsContent');
      if (drill.model_sentence && drill.model_sentence.trim() && grammarHints && hintsContent) {
        let hintsHtml = '';
        // Show cues if available and different from model sentence
        if (drill.cues && drill.cues.length > 0) {
          const uniqueCues = drill.cues.filter(cue => cue !== drill.model_sentence);
          if (uniqueCues.length > 0) {
            hintsHtml += `<div class="fsi-cues"><strong>Cues:</strong> ${uniqueCues.join(', ')}</div>`;
          }
        }
        // Show expected response if available (user needs to produce this)
        if (drill.expected_responses && drill.expected_responses.length > 0) {
          const expected0 = fillFsiBlankSlots(drill.expected_responses[0]);
          hintsHtml += `<div class="fsi-expected"><strong>Produce:</strong> ${expected0}</div>`;
        }
        // Optional debug info for troubleshooting repair logic
        if (FSI_DrillFormats?.debug && drill.fsi_repair) {
          const r = drill.fsi_repair;
          hintsHtml += `<div class="fsi-debug"><strong>Debug:</strong> ${r.source}/${r.match} ${r.exercice_id}-${String(r.drill_number).padStart(2,'0')}</div>`;
        }
        if (hintsHtml) {
      ...
      } else if (grammarHints) {
        grammarHints.style.display = 'none';
      }
    } else {
       drillTypeElement.classList.remove('fsi-drill', 'fsi-substitution', 'fsi-transformation', 'fsi-variation', 'fsi-response', 'fsi-review', 'fsi-lexical', 'fsi-question');
    }

    let frenchText = drill.french;
    // For FSI drills, french might be empty - use first expected response
    if (drill.type && drill.type.startsWith('fsi_') && (!frenchText || frenchText === 'N/A') && drill.expected_responses && Array.isArray(drill.expected_responses) && drill.expected_responses.length > 0) {
      frenchText = drill.expected_responses[0];
    }
    
    // For FSI drills with model sentence, show model as prompt instead of expected response
    if (drill.type && drill.type.startsWith('fsi_')) {
      if (drill.model_sentence && drill.model_sentence.trim()) {
        frenchText = drill.model_sentence;
      } else if (drill.cues && drill.cues.length > 0) {
        // If no model sentence but has cues, show first cue as prompt
        frenchText = drill.cues[0];
      }
    }
    if (drill.type && drill.type.startsWith('fsi_')) {
      frenchText = fillFsiBlankSlots(frenchText);
    }
  ...
  promptFr.textContent = frenchText;
   // For FSI drills with metadata English text, show appropriate instruction
   let englishPrompt = drill.english || '(translate to French)';
   if (drill.type && drill.type.startsWith('fsi_') && drill.english && drill.english.trim().startsWith('[')) {
     // Show drill-type-specific instruction
     const fsiType = drill.type.replace('fsi_', '');
     switch (fsiType) {
       case 'substitution':
         englishPrompt = 'Make substitution using cue';
         break;
       case 'transformation':
         englishPrompt = 'Transform the sentence';
         break;
       case 'variation':
         englishPrompt = 'Create variation';
         break;
       case 'response':
         englishPrompt = 'Respond appropriately';
         break;
       case 'review':
         englishPrompt = 'Review exercise';
         break;
       case 'lexical':
         englishPrompt = 'Lexical exercise';
         break;
       case 'question':
         englishPrompt = 'Answer the question';
         break;
       default:
         englishPrompt = 'FSI exercise';
     }
   }
   promptEn.textContent = englishPrompt;
  ...
  if (drillMode === 'translate') {
    ...
    } else if (drill.english && drill.english.trim() && !(drill.type && drill.type.startsWith('fsi_') && drill.english.trim().startsWith('['))) {
    ...
    // FSI drill hints are handled separately above
  } else {
...
function checkAnswer() {
  const input = document.getElementById('userInput');
  const drill = currentDrills[currentDrillIndex];
  const isRepeatInstance = drill?.is_repeat === true;

   let expected;
   let expectedResponses = [];
   
   if (drillDirection === 'fr-en') {
     // English answers should not use French error-classification.
     if (!hasUsableEnglish(drill)) {
       // Auto-switch to EN→FR for drills without reliable English prompts.
       autoDirectionGuard = true;
       setDirection('en-fr');
       autoDirectionGuard = false;
       input.value = '';
       ...
       feedbackTitle.textContent = 'Not available';
       feedbackDetail.textContent = 'This drill does not have a reliable English prompt. Switched to EN→FR.';
       return;
     }
     expected = drill.english;
     expectedResponses = [drill.english];
   } else {
     // For FSI drills, check for expected_responses array
     if (drill.type && drill.type.startsWith('fsi_') && drill.expected_responses && Array.isArray(drill.expected_responses) && drill.expected_responses.length > 0) {
       expectedResponses = drill.expected_responses.map(fillFsiBlankSlots);
       expected = expectedResponses[0]; // Use first as primary for display
     } else {
       expected = drill.french;
       expectedResponses = [drill.french];
     }
     if (register === 'informal') {
       expected = convertToTu(expected);
       expectedResponses = expectedResponses.map(resp => convertToTu(resp));
     }
   }

   // Check against all expected responses for FSI drills
   let result;
   if (expectedResponses.length > 1) {
     // Check each expected response
     let bestMatch = null;
     for (const exp of expectedResponses) {
       const testResult = drillDirection === 'fr-en' ? EnglishCompare(input.value, exp) : FSI_Error.classify(input.value, exp);
       if (testResult.correct) {
         bestMatch = testResult;
         bestMatch.matchedExpected = exp;
         break;
       }
       // Keep track of best match? For now just use first
     }
     if (bestMatch) {
       result = bestMatch;
       expected = bestMatch.matchedExpected || expected; // Update expected to matched one
     } else {
       // No match, use first expected for error reporting
       result = drillDirection === 'fr-en' ? EnglishCompare(input.value, expected) : FSI_Error.classify(input.value, expected);
     }
   } else {
     result = drillDirection === 'fr-en' ? EnglishCompare(input.value, expected) : FSI_Error.classify(input.value, expected);
   }
  ...
  if (retryMode) {
    if (result.correct) {
      // Got it right on retry - show success and auto-advance
      ...
      // Still wrong - just clear and let them try again
  ...
  if (result.correct) {
    ...
    if (!isRepeatInstance && typeof FSI_SRS !== 'undefined') {
    ...
    // Save to cloud
    if (!isRepeatInstance && typeof FSI_Auth !== 'undefined') {
    ...
    // Linear mode progress tracking
    if (!isRepeatInstance && linearDrillSession && typeof FSI_Linear !== 'undefined') {
      FSI_Linear.state.current_item = currentDrillIndex;
      FSI_Linear.markComplete(true);
    }
...
// Close drill view
function closeDrill() {
  linearDrillSession = false;  // Reset linear drill session flag
  document.getElementById('drillView').classList.remove('active');
